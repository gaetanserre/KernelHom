...
def kernelReassocHandler (e : Expr) : MetaM (Expr × Array MVarId) := do
  ...
  let some (_, lhs, rhs) := e_type.eq? | throwError "kernelReassocHandler: l'expression n'est pas une égalité"
  logInfo m!"kernelReassocHandler: {lhs}"
  let (X, Y, xLvl, yLvl) ← getTypesFromKernel lhs
  logInfo m!"kernelReassocHandler: {X}, {Y}, {xLvl}, {yLvl}"
  let (_, e_proof) ← HomEquality e_type
  let e_proof ← mkAppM ``Eq.mp #[e_proof, e]
  logInfo m!"kernelReassocHandler: {e_proof}"
  let (ee, _) ← reassocExprHom e_proof
  logInfo m!"kernelReassocHandler2: {ee}"
  return (e, #[])
