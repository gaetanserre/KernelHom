...
partial def transformKernelToHomQ' {u v : Level} {α : Q(Type u)} {β : Q(Type v)}
  ...
  | ~q(@Kernel.monoComp _ $X $Y _ _ $mX $mY _ $m_coherence $κ $mκ $η $mη) =>
    ...
    return ⟨lvl, α', β', eα', eβ', q($hom_κ ⊗≫ $hom_η)⟩
  | ~q($κ) =>
    logInfo m!"Matched kernel: {κ}"
    let ex : Q(ULift.{max u v maxLvl} $α ≃ᵐ $α) := q(ulift.{u, max u v maxLvl} (α := $α))
    let ey : Q(ULift.{max u v maxLvl} $β ≃ᵐ $β):= q(ulift.{v, max u v maxLvl} (α := $β))
