...
Universe handling is part of this translation: expressions are lifted to a common universe level, so rewrites stay well-typed across universe levels. This part is handled by the `eq_lift` tactic, which can also be used independently (see [the GitHub repository](https://github.com/gaetanserre/KernelLift)).
