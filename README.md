...
- **Reasoning by calculation.** A proof can be written as a `calc` whose lines are the mathematically meaningful rewrites, while the structural steps between them are proved automatically by the categorical tactics. Each step can moreover be visualized. Such proofs by calculation were rare for kernels, since each structural step needed its own lemma or a computation with integrals. See [`KernelHomTests/Basu.lean`](KernelHomTests/Basu.lean), which formalizes Theorem 15.8 of Fritz, the main ingredient of Basu's theorem, and the [calculational proofs page](https://gaetanserre.fr/KernelHom/Calculational-proofs/) of the documentation, which draws each step of the calculation.
