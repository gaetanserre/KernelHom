...
These tactics allow users to transform complex kernel equalities into categorical equalities, where powerful categorical tactics can be applied to simplify or prove them. To this end, the library provides built-in helpers like [`kernel_coherence`](doc/KernelHom/Tactic/KernelCat.html#tacticKernel_coherence) and [`kernel_monoidal`](doc/KernelHom/Tactic/KernelCat.html#tacticKernel_monoidal) to apply categorical tactics directly to kernels without needing to manually invoke the translation tactics.
